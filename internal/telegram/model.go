...
// TelegramProfileWithUser extends TelegramProfile with user data for reminders
type TelegramProfileWithUser struct {
	Id                  int64      `json:"id" db:"id"`
	Token               string     `json:"token" db:"token"`
	TelegramId          *string    `json:"telegramId" db:"telegram_id"`
	TelegramUsername    *string    `json:"telegramUsername" db:"telegram_username"`
	...
	CreatedAt           time.Time  `json:"-" db:"created_at"`
	UpdatedAt           time.Time  `json:"-" db:"updated_at"`
	LastDailyReminderAt *time.Time `json:"-" db:"last_daily_reminder_at"`
	UserTimezone        *string    `json:"-" db:"user_timezone"`
	UserLocale          *string    `json:"-" db:"user_locale"`
}
