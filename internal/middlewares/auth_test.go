...
func TestIsPublicAuthRoute(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "check username", method: http.MethodGet, path: "/api/v1/auth/check-username/somivyn", want: true},
		{name: "check username with trailing slash", method: http.MethodGet, path: "/api/v1/auth/check-username/somivyn/", want: true},
