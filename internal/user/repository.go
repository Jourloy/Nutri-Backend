...
func (r *repository) DeleteUser(ctx context.Context, id string) (*User, error) {
	...
	deletions := []struct {
		...
		{query: `DELETE FROM body_cycle_daily_logs WHERE user_id = $1`, args: []any{id}},
		{query: `DELETE FROM body_cycles WHERE user_id = $1`, args: []any{id}},
		{query: `DELETE FROM body_activity WHERE user_id = $1`, args: []any{id}},
		{query: `DELETE FROM body_workouts WHERE user_id = $1`, args: []any{id}},
