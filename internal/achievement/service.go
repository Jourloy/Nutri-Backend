...
func (s *service) UpdateCategory(ctx context.Context, c Category) (*Category, error) {
...
func (s *service) CreateAchievement(ctx context.Context, a Achievement) (*Achievement, error) {
	return s.repo.CreateAchievement(ctx, a)
}
func (s *service) UpdateAchievement(ctx context.Context, a Achievement) (*Achievement, error) {
	return s.repo.UpdateAchievement(ctx, a)
}
...
func (s *service) ListForUser(ctx context.Context, userId string) ([]AchievementView, error) {
	...
	for _, a := range achs {
		...
		uv := AchievementView{
			...
			Key:            a.Key,
			Name:           a.Name,
			NameKey:        "achievement." + a.Key + ".name",
			Description:    a.Description,
			DescriptionKey: "achievement." + a.Key + ".description",
			Icon:           a.Icon,
			Color:          a.Color,
			Points:         a.Points,
			IsSecret:       a.IsSecret,
		...
		if a.CategoryId != nil {
			if c, ok := catById[*a.CategoryId]; ok {
				...
				categoryNameKey := "achievement.category." + c.Key + ".name"
				uv.CategoryNameKey = &categoryNameKey
			}
		}
		if _, ok := unlocked[a.Id]; ok {
			...
			if a.IsSecret {
				...
				uv.DescriptionKey = "achievement.secret"
				uv.Requirement = 1
				uv.Current = 0
			}
		}
...
func (s *service) currentStreakDays(ctx context.Context, userId string) int {
	...
	m := map[string]struct{}{}
	for _, r := range days {
		m[r.D.Format("2006-01-02")] = struct{}{}
	}
