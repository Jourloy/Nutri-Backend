...
func (r *repository) CreateAchievement(ctx context.Context, a Achievement) (*Achievement, error) {
	...
	if len(out.CriteriaRaw) > 0 {
		_ = json.Unmarshal(out.CriteriaRaw, &out.Criteria)
	}
	return &out, nil
}
...
func (r *repository) GetAchievements(ctx context.Context) ([]Achievement, error) {
	...
	for i := range res {
		if len(res[i].CriteriaRaw) > 0 {
			_ = json.Unmarshal(res[i].CriteriaRaw, &res[i].Criteria)
		}
	}
	return res, nil
}
